...
private func delayForImage(at index: Int, source: CGImageSource) -> Double {
    ...
    if delayWrapper.doubleValue == 0 { delayWrapper = unsafeBitCast(CFDictionaryGetValue(gifProperties, Unmanaged.passUnretained(kCGImagePropertyGIFDelayTime).toOpaque()), to: AnyObject.self) }
...
struct GIFImageTest: View {
    ...
    var body: some View {
        VStack { GIFImage(name: "preview") .frame(height: 300)
            if let data = imageData { GIFImage(data: data) .frame(width: 300)
                if isDone { Text("Done")
                } else {
                    GIFImage(data: data, repetitions: 2) { isDone = true
                    } .frame(width: 300)
                }
            } else {
    ...
    private func loadData() {
        let task = URLSession.shared.dataTask(with: URL(string: "https://github.com/globulus/swiftui-webview/raw/main/Images/preview_macos.gif?raw=true")!) { data, response, error in imageData = data
        }
        task.resume()
    }
}
